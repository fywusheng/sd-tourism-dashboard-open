...
const mapData = computed(() => corePanelData.value.mapData || [])
const mapOption = ref({})

// const placeList = ref([
//   { name: '海门', geoCoord: [121.15, 31.89], rank: 25 },
...
// ])
// console.log('placeList', JSON.stringify(placeList.value))
// const mapOption = ref(getMapOption(mapData.value))

watch(
  () => mapData.value,
  newVal => {
    if (newVal.length === 0) return
    mapOption.value = getMapOption(newVal)
  },
  { immediate: true }
)
</script>
